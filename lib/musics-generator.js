...
//                console.log(response.body);
                ...
                }).parseFromString(response.toString());
                ...
                if (next.startsWith("/")) {
                    next = "https://music.douban.com" + next;
                }
                ...
                for (var i in items) {
                    ...
                    var comment = xpath.select1('string(//li[4])', parser);
...
module.exports = function (locals) {
    ...
    if (!config.douban || !config.douban.music) {//当没有输入movie信息时，不进行数据渲染。
        return;
    }
    ...
    var watchedUrl = 'https://music.douban.com/people/' + config.douban.user + '/collect';
    ...
    var watchingUrl = 'https://music.douban.com/people/' + config.douban.user + '/do';
    ...
    log.info(wish.length + watched.length + ' musics have been loaded in ' + (endTime - startTime) + " ms" + offlinePrompt);

    var __ = i18n.__(config.language);

    var contents = ejs.renderFile(path.join(__dirname, 'templates/music.ejs'), {
        'quote': config.douban.music.quote,
        'wish': wish,
        'listen': watched,
        'listening': watching,
        '__': __,
    ...
    return {
        ...
        data: {
            title: config.douban.music.title,
            content: contents,
            slug: 'musics'
        },
        layout: ['page', 'post']
